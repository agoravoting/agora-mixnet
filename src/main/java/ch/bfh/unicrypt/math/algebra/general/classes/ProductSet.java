...
public class ProductSet
	...
	protected <W> Tuple defaultGetElementFrom(Tree<W> tree, ConvertMethod<W> convertMethod) throws UniCryptException {
		// FIXME remove
		if(debug) {
System.out.println("productset: tree " + tree);
	...
	// drb
	protected <W> Tuple defaultGetElementFrom(Tree<W> tree, ConvertMethod<W> convertMethod, boolean unsafe) throws UniCryptException {
		if (tree.isLeaf()) {
			throw new UniCryptException(ErrorCode.ELEMENT_CONVERSION_FAILURE);
		}
		Node<W> node = (Node<W>) tree;
		...
		int i = 0;
		for (Tree<W> child : node.getChildren()) {
			elements[i] = ((AbstractSet) this.getAt(i)).getElementFrom(child, convertMethod, unsafe);
			i++;
		}
		return this.getElement(DenseArray.getInstance(elements), unsafe);
	}
