...
public class ReEncryptionShuffleProofSystem
	...
	protected Tuple abstractGenerate(Triple privateInput, Tuple publicInput, RandomByteSequence randomByteSequence) {
		...
		PreimageProofFunction f
			   = new PreimageProofFunction(this.getCyclicGroup(), this.getSize(), this.getResponseSpace(),
										   this.getCommitmentSpace(), this.getIndependentGenerators(), uPrimeV, this.encryptionScheme,
										   this.encryptionPK);

		Tuple randomElement = this.getResponseSpace().extractPrefix(2).getRandomElement(randomByteSequence);
		Tuple randEV = ProductGroup.getInstance(ZMod.getInstance(MathUtil.powerOfTwo(this.getKe() + this.getKc() + this.getKr())),
	...
	public static ReEncryptionShuffleProofSystem getInstance(SigmaChallengeGenerator sigmaChallengeGenerator,
		...
		Tuple independentGenerators2 = Tuple.getInstance(((AbstractCyclicGroup) elgamal.getCyclicGroup()).getIndependentGeneratorsParallel(randomByteSequence, 0, size + 1));
		System.out.println(System.currentTimeMillis() - now); now = System.currentTimeMillis();
		Tuple independentGenerators = Tuple.getInstance(((AbstractCyclicGroup) elgamal.getCyclicGroup()).getIndependentGeneratorsP(randomByteSequence, size + 1));
