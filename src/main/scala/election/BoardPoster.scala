      ...
      futureResponse onSuccess { case response =>
       response.json.validate[BoardAttributes] match {
         case attr: JsSuccess[BoardAttributes] =>
           println("Election creation posted to immutable log. Election id is " + attr.get.index + " and uid " + election.state.uid)
           // The post message index will be the unique id of the election
           promise.success(new Election[W, Created](Created(attr.get.index, election.state.cSettings, election.state.uid, election.state.dto)))
         case JsError(e) =>
